...
curl_setopt($ch, CURLOPT_URL, "https://api.mysportsfeeds.com/v2.1/pull/nhl/latest/games/".$_GET['latestGameId']."/boxscore.json");
...
if (!$resp && curl_errno($ch) === 0) {
    unset($resp);
    curl_setopt($ch, CURLOPT_URL, "https://api.mysportsfeeds.com/v2.1/pull/nhl/latest/games/".$_GET['backupGameId']."/boxscore.json");
    $resp = curl_exec($ch);
} elseif (!$resp) {
